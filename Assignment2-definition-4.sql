...
CREATE TABLE client (
  ...
  dob DATE,
  CONSTRAINT unique_Client UNIQUE(first_name, last_name),
  PRIMARY KEY (id)
...
INSERT INTO project (cid, name, notes) VALUES (
	(SELECT id FROM client WHERE first_name = 'Sara' && last_name = 'Smith'), 'Diamond', 'Should be done by Jan 2017');
-- cid - reference to first_name: David last_name: Atkins
-- name - Eclipse
-- notes - NULL
INSERT INTO project (cid, name) VALUES (
	(SELECT id FROM client WHERE first_name = 'David' && last_name = 'Atkins'), 'Eclipse');
-- cid - reference to first_name: Daniel last_name: Jensen
-- name - Moon 
-- notes - NULL
INSERT INTO project (cid, name) VALUES (
	(SELECT id FROM client WHERE first_name = 'Daniel' && last_name = 'Jensen'), 'Moon');
